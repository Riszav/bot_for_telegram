...
async def list_referal_call(call: types.CallbackQuery):
    db = Database()
    referal_users = db.sql_select_referals_by_owner_user(
        telegram_id=call.from_user.id
    )
    print(referal_users)
...
def register_reference_handlers(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(list_referal_call,
                                       lambda call: call.data == "list_referal")